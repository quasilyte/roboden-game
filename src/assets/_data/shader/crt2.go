...
func Fragment(dst vec4, src vec2, color vec4) vec4 {
	...
	var col vec3
	col.r = imageSrc0At(vec2(uv.x+0.0001, uv.y+0.0001)*size+origin).x + 0.005
	col.g = imageSrc0At(vec2(uv.x+0.0000, uv.y-0.0002)*size+origin).y + 0.005
	col.b = imageSrc0At(vec2(uv.x-0.0002, uv.y+0.0000)*size+origin).z + 0.005
	...
	col *= (1 - 0.25*vec3(clamp((mod(src.x, 2)-1)*2, 0, 1)))
