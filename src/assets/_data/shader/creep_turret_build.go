...
func Fragment(position vec4, texCoord vec2, color vec4) vec4 {
	...
	if dist > (24.0 * (1.4 - Time)) {
		return gridPattern(v, vec4(1.5, 1.1, 1.0, 1.0), 15, actualPixPos, pixSize, originTexPos)
	}
	if dist > (18.0 * (1.4 - Time)) {
		return gridPattern(v, vec4(1.9, 1.2, 0.9, 1.0), 12, actualPixPos, pixSize, originTexPos)
	}
	if dist > (12.0 * (1.4 - Time)) {
		return gridPattern(v, vec4(2.3, 1.4, 0.8, 1.0), 8, actualPixPos, pixSize, originTexPos)
	}
